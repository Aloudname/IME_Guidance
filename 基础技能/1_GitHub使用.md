...
有了仓库，我们就可以依托它来开发程序、管理版本了。先认识一下仓库：仓库分为 **远程仓库** 和 **本地仓库**。远程仓库就是存储在 GitHub 上面的这些仓库；本地仓库就是存储在本地（包括我们的电脑、课题组服务器甚至安装 VScode for Android 服务的平板等）的仓库。值得一提，我们并不能直接对仓库里的内容进行更改。换句话说，仓库对外不可修改，完全是 “针插不进水泼不进”。因此，还需要通过一个叫 “**工作区**” 的东西做本地仓库对外的接口。为什么要这样设计呢？其实是为了避免直接修改本地仓库造成的高频次仓库访问、仓库更新频繁、版本管理混乱等问题。说到这你可能就明白了：我们平时写程序，直接修改的对象其实是本地的工作区，需要另外将对工作区的修改应用到本地仓库中，并且另外将本地仓库的更改同步到远程仓库中。那么问题来了：

-[] 如何根据远程仓库建立一个本地仓库（以及工作区）呢？（创建本地仓库）
-[] 如何让本地仓库的内容、进度与远程仓库保持一致呢？（更新本地仓库）
...
仍然点击在 GitHub 仓库主页面的绿色按钮 `Code`。可以看到有三种不同格式的、用于 Clone 的路径：
...
分别对应三种创建本地仓库的方式。我们在命令行中进入本地仓库要存储的文件夹，选择其中一种方式运行：
...
本地仓库不会主动更新自己以保持与远程仓库的同步。也就是说，本地仓库和远程仓库之间没有自动同步的关系。如果本地仓库来自于其他人的远程仓库，或者仓库是协同开发的项目时，必须非常重视本地仓库和远程的同步。
...
假若我们在本地的项目文件夹中新建了一个 `hello_world.py` 、修改了原先已经存在的 `nihao.py` 并删除了原先存在的 `konichiwa.py`，做了一个增、删、改都有的更改。该如何将这次更改更新到本地仓库呢？

#### 3.1 编辑器的图形化界面

我们前面已经在代码编辑器中下载了 Git 项目管理的拓展。他们提供了图形化界面，管理非常方便。
[WIP]
